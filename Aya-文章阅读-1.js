...
if (jc_timebody) {
    ...
    var fs = require("fs");
    jc_timebody = fs.readFileSync("Any-1-1/jc_timebody.txt", "utf8");
    if (jc_timebody !== `undefined`) {
        jc_timebodys = jc_timebody.split("\n");
    } else {
...
Object.keys(jc_timebodys).forEach((item) => {
    if (jc_timebodys[item] && !jc_timebodys[item].startsWith("#")) {
...
if (wzbody) {
    ...
    var fs = require("fs");
    wzbody = fs.readFileSync("Any-1-1/jc_wzbody.txt", "utf8");
    if (wzbody !== `undefined`) {
        wzbodys = wzbody.split("\n");
    } else {
...
Object.keys(wzbodys).forEach((item) => {
    if (wzbodys[item] && !wzbodys[item].startsWith("#")) {
