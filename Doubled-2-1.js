...
if (zq_cookie) {
    ...
    var fs = require("fs");
    zq_cookie = fs.readFileSync("Cohc-2-1/zq_cookie.txt", "utf8");
    ...
    getzq_cookie()
     $.done()
         ...
         for (let k = 0; k < zq_cookieArr.length; k++) {
             ...
             zq_cookie1 = '&device_brand=xfdg&device_id=cc7dgdsgfsz83e&device_model=1gx&device_platform=android&device_type=android&inner_version=202107261526&mi=0&openudid=cc7dgdsgfsz83e&os_api=27&os_version=bdftgsdfga&phone_network=WIFI&phone_sim=1' + '&request_time=' + time1 + '&' + bodyVal
             //待处理cookie
             console.log(`${zq_cookie1}`)
             console.log(`--------第 ${k + 1} 个账号阅读翻倍中--------\n`)
         ...
         if (message.length != 0) {
             await notify ? notify.sendNotify("中青看点阅读翻倍", `${message}\n\n 吹水群：https://t.me/ShaolinTemple2`) :
                 $.msg($.name, "中青看点阅读翻倍", `${message}\n\n吹水群：https://t.me/ShaolinTemple2`);
         } else if ($.isNode()) {
             await notify.sendNotify("中青看点阅读翻倍", `${message}\n\n吹水群：https://t.me/ShaolinTemple2`);
         }
     }
     })()
    .catch((e) => $.logErr(e))
...
function today_score(zq_cookie1,timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url : 'https://kandian.wkandian.com/v17/NewTask/setJoinTaskPromotion.json?'+ zq_cookie1,
            headers : {
